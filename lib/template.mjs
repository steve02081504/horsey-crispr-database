...
/**
 * 定位紧跟在 `${` 之后的 JS 表达式末尾（返回值为 `}` 之后下一个字符的下标；表达式本体为 src.slice(0, 返回值 - 1)）。
 * 处理字符串、行/块注释，以及模板字符串内嵌套的 `${...}`，避免把内层 `}` 误认为外层结束。
 * @param {string} src - 已去掉开头的 `${`，从表达式第一个字符开始
 * @returns {number} - 失败时为 -1
 */
function findTemplateExpressionEnd(src) {
	let i = 0
	let braceDepth = 0
	/** @type {'code' | 'sq' | 'dq' | 'tmpl'} */
	let state = 'code'
	...
	while (i < src.length) {
		...
		if (state === 'tmpl') {
			...
			if (c === '$' && src[i + 1] === '{') {
				const innerEnd = findTemplateExpressionEnd(src.slice(i + 2))
...
export function usingTemplates(path) {
	templatePath = new URL(path.replace(/\/$/, '') + '/', base_dir + '/').href.replace(/\/$/, '')
}
...
export async function renderTemplateNoScriptActivation(template, data = {}) {
	...
	// 使用循环匹配所有 ${...} 表达式（结束符由 findTemplateExpressionEnd 解析，支持嵌套模板字符串）
	let result = ''
	const errors = []
	while (html.indexOf('${') != -1) {
		...
		const endLen = findTemplateExpressionEnd(html)
		if (endLen < 0) {
			console.error('Unclosed or invalid ${...} in template')
			result += '${' + html
			break
		}
		const expression = html.slice(0, endLen - 1)
		try {
	...
	if (errors.length) errors.forEach(console.error)
	result += html
